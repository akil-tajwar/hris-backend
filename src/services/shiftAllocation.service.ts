  ...
  const existing = await db
    ...
    .where(
      and(
        ...
        eq(employeeShiftAllocations.effectiveFrom, toDateString(data.effectiveFrom))
      )
    )
    .limit(1)
...
export const createBulkShiftAllocation = async (
  ...
  if (missingIds.length)
    throw new Error(`এই employee ID গুলো পাওয়া যায়নি: ${missingIds.join(', ')}`)
  ...
  return {
    ...
    employees:      employees.map((e) => ({ id: e.employeeId, name: e.empFullName })),
...
export const copyAllActiveAllocations = async (
  ...
  if (!filteredRows.length)
    throw new Error(
      `সব employee এর পরবর্তী period এ allocation ইতিমধ্যে আছে`
    )
...
// // ─── Date helpers ─────────────────────────────────────────────────
// // এটা ব্যবহার করুন — timezone safe:
// const toDateString = (date: string | Date): string => {
//   if (typeof date === 'string') return date.slice(0, 10)
//   const y = date.getFullYear()
//   const m = String(date.getMonth() + 1).padStart(2, '0')
...
//   const [result] = await db
//     .insert(employeeShiftAllocations)
//     .values({
//       employeeId:       data.employeeId,
//       shiftId:          data.shiftId,
//       effectiveFrom:    toDateString(data.effectiveFrom),
...
//       and(
//         inArray(employeeShiftAllocations.employeeId, employeeIds),
//         eq(employeeShiftAllocations.effectiveFrom, toDateString(rest.effectiveFrom))
//       )
//     )
...
// // ─── COPY SINGLE ──────────────────────────────────────────────────
// export const copyShiftAllocation = async (
//   id: number,
//   createdBy: number,
// ) => {
//   const existing = await db.query.employeeShiftAllocations.findFirst({
...
//   const dateRange =
//     existing.recurrenceType === 'weekly'
//       ? getNextWeekRange(existing.effectiveFrom)
//       : getNextMonthRange(existing.effectiveFrom)
...
// //   const latestFrom = activeAllocations.reduce((latest, curr) =>
// //     curr.effectiveFrom > latest ? curr.effectiveFrom : latest,
// //     activeAllocations[0].effectiveFrom
// //   )

// //   const dateRange =
// //     recurrenceType === 'weekly'
// //       ? getNextWeekRange(latestFrom)
// //       : getNextMonthRange(latestFrom)

// //   // Duplicate check — যেগুলো ইতিমধ্যে আছে বাদ দাও
// //   const employeeIds = activeAllocations.map((a) => a.employeeId)
...
// //   const existingEmployeeIds = new Set(alreadyExists.map((e) => e.employeeId))
// //   const filteredAllocations = activeAllocations.filter(
// //     (a) => !existingEmployeeIds.has(a.employeeId)
// //   )

// //   if (!filteredAllocations.length) {
// //     throw new Error(`সব employee এর ${dateRange.effectiveFrom} তারিখে allocation ইতিমধ্যে আছে`)
// //   }
...
// //     dateRange,
// //     message:        `${rows.length} টি allocation copy হয়েছে${existingEmployeeIds.size > 0 ? `, ${existingEmployeeIds.size} টি skip হয়েছে (duplicate)` : ''}`,
// //   }
// // }
// export const copyAllActiveAllocations = async (
//   recurrenceType: 'weekly' | 'monthly',
...
//   // প্রতিটা employee এর সবচেয়ে latest allocation খুঁজে বের করো
//   const latestPerEmployee = new Map<number, typeof activeAllocations[0]>()
...
//   // প্রতিটার জন্য আলাদাভাবে next date calculate করো
//   const rows = latestAllocations.map((alloc) => {
//     const dateRange =
//       recurrenceType === 'weekly'
//         ? getNextWeekRange(alloc.effectiveFrom)
//         : getNextMonthRange(alloc.effectiveFrom)
...
//   // Duplicate check — already exist করলে skip
//   const employeeIds = rows.map((r) => r.employeeId)
//   const fromDates   = [...new Set(rows.map((r) => r.effectiveFrom))]
...
//     alreadyExists.map((e) => `${e.employeeId}`)
//   )
